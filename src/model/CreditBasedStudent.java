...
public class CreditBasedStudent extends Student {
    ...
    public boolean registerCourse(Course course) {
        ...
        if (!program.containsCourse(course.getCourseID())) return false;
        // Kiểm tra điều kiện tiên quyết
        if (!checkPrerequisites(course)) return false;
        return enrollCourse(course);
    }

    /**
     * Tính CPA: quy đổi từng học phần đã nhập điểm sang thang 4 rồi tính trung bình có trọng số.
     * Bao gồm cả những môn chưa pass (điểm <4 sẽ quy thành 0).
     *
     * @return CPA thang 4
     */
    @Override
    public double calculateFinalGrade() {
        // Lấy tất cả các Enrollment đã nhập đủ điểm
