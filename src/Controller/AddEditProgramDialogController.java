...
public class AddEditProgramDialogController {
    ...
    @FXML private ComboBox<ProgramType> comboProgramType;
    @FXML private Label lblElectiveCreditReq;
    @FXML private TextField txtElectiveCreditReq;
    @FXML private Label lblTotalCreditReq;
    @FXML private TextField txtTotalCreditReq;

    @FXML private ListView<String> listRequiredCourses;
    @FXML private TextField txtRequiredCourseIdInput;
    // Các nút đã có onAction trong FXML, không cần @FXML ở đây trừ khi bạn muốn truy cập trực tiếp
    // @FXML private Button btnAddRequiredCourse;
    // @FXML private Button btnRemoveRequiredCourse;

    @FXML private VBox electiveCoursesSection; // fx:id cho VBox chứa phần tự chọn
    @FXML private ListView<String> listElectiveCourses;
    @FXML private TextField txtElectiveCourseIdInput;
    ...
    private void handleSave() {
        ...
        if ((!isEditMode && programManager.findProgram(majorName) != null) ||
            (isEditMode && !originalMajorNameForEditMode.equalsIgnoreCase(majorName) && programManager.findProgram(majorName) != null)) {
        ...
        if (type == ProgramType.CREDIT_BASED) {
            if (txtElectiveCreditReq.getText().trim().isEmpty() || txtTotalCreditReq.getText().trim().isEmpty()) {
                lblValidationMessage.setText("Với hệ Tín chỉ, vui lòng nhập đủ Tín Chỉ Tự Chọn Yêu Cầu và Tổng Tín Chỉ Yêu Cầu.");
                return;
            }
            try {
                ...
                if (actualRequiredCourses.isEmpty() && totalCreditReq > 0 && electiveCreditReq < totalCreditReq) {
                     lblValidationMessage.setText("Chương trình tín chỉ cần có học phần bắt buộc nếu Tổng Tín Chỉ Yêu Cầu > Tín Chỉ Tự Chọn Yêu Cầu.");
                    return;
                }
        ...
        List<Course> finalElectiveCourses = (type == ProgramType.CREDIT_BASED) ? new ArrayList<>(actualElectiveCourses) : new ArrayList<>();
        ...
        if (type == ProgramType.PART_TIME) {
            ...
            if (sumReqCreditsFromList != expectedReqFromInput) {
                lblValidationMessage.setText("Tổng Tín Chỉ Bắt Buộc (" + sumReqCreditsFromList + ") không khớp (Tổng Tín Chỉ Yêu Cầu - Tín Chỉ Tự Chọn Yêu Cầu = " + expectedReqFromInput + ").");
                return;
            }
            if (sumElecCreditsFromList < electiveCreditReq) {
                 lblValidationMessage.setText("Tổng Tín Chỉ Tự Chọn trong danh sách (" + sumElecCreditsFromList + ") nhỏ hơn Tín Chỉ Tự Chọn Yêu Cầu (" + electiveCreditReq + ").");
                return;
            }
        }
        ...
        if (isEditMode) {
            ...
            if (successOperation) {
                showAlert(Alert.AlertType.INFORMATION, "Thành công", "Cập nhật chương trình '" + majorName + "' thành công.");
            } else {
                 showAlert(Alert.AlertType.ERROR, "Lỗi", "Cập nhật chương trình thất bại. Kiểm tra Console để biết thêm chi tiết.");
                 return;
            }
        } else {
            Program newProgram = new Program(majorName, type,
                                            finalRequiredCourses,
                                            finalElectiveCourses,
                                            electiveCreditReq, totalCreditReq);
            successOperation = programManager.addProgram(newProgram);
            if (successOperation) {
                showAlert(Alert.AlertType.INFORMATION, "Thành công", "Thêm chương trình mới '" + majorName + "' thành công.");
            } else {
