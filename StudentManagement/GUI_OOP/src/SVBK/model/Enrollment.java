...
/**
 * Đại diện một bản ghi đăng ký học phần và điểm số của sinh viên. Mỗi
 * Enrollment gắn liền với một Course, lưu trữ điểm giữa kỳ và cuối kỳ.
 */
public class Enrollment {
	...
	/**
	 * Kiểm tra xem học phần đã đạt hay chưa (điểm tổng kết >= 4.0). Phải đã nhập đủ
	 * điểm và calculateFinalGrade() >= 4.0.
	 * 
	...
	/**
	 * Tính điểm tổng kết môn dựa trên điểm Enrollment và trọng số của Course. grade
	 * = midterm * (1 - finalWeight) + final * finalWeight. Nếu chưa hoàn thành, trả
	 * về 0.0.
	 *
	...
	public double calculateFinalGrade() {
		...
		return midtermScore * (1.0 - weight) + finalScore * weight;
	}

	@Override
	public String toString() {
		return String.format("%s - %s | GK: %s | CK: %s | KQ: %.2f", course.getCourseID(), course.getCourseName(),
				midtermScore == null ? "--" : midtermScore.toString(),
				finalScore == null ? "--" : finalScore.toString(), calculateFinalGrade());
	}
