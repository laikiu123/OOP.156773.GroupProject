...
/**
 * Sinh viên hệ tín chỉ. Điểm học phần tính theo thang 10, sau đó quy đổi từng
 * học phần sang thang 4 rồi tính CPA (trung bình có trọng số theo tín chỉ) trên
 * thang 4.
 */
public class CreditBasedStudent extends Student {
	...
	public boolean checkPrerequisites(Course course) {
		...
		return getCompletedEnrollments().stream().anyMatch(e -> e.getCourse().getCourseID().equalsIgnoreCase(preID));
	}
	 ...
	 * @return true nếu thêm mới; false nếu khóa không thuộc chương trình hoặc chưa
	 *         thỏa prerequisite hoặc đã đăng ký trước đó
	 */
	...
	/**
	 * Tính CPA: quy đổi từng học phần đã hoàn thành sang thang 4 rồi tính trung
	 * bình có trọng số.
	 *
	 * @return CPA thang 4
	 */
	...
	/**
	 * Kiểm tra điều kiện tốt nghiệp: 1) Hoàn thành tất cả requiredCourses 2) Tích
	 * lũy đủ tín chỉ tự chọn 3) Tích lũy đủ tổng tín chỉ chương trình
	 *
	...
	public boolean checkGraduation() {
		...
		// 3) total credits
		int totalCredits = getCompletedEnrollments().stream().mapToInt(e -> e.getCourse().getCreditCount()).sum();
