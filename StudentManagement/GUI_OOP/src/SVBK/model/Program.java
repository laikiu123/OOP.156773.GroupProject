...
/**
 * Đại diện cho chương trình đào tạo của một ngành học. Chứa thông tin về loại
 * chương trình (tín chỉ hoặc niên chế), danh sách học phần bắt buộc, tự chọn và
 * các ngưỡng tín chỉ.
 */
public class Program {
	 ...
	 * @param type                      Loại chương trình (CREDIT_BASED hoặc
	 *                                  PART_TIME)
	 ...
	 */
	public Program(String majorName, ProgramType type, List<Course> requiredCourses, List<Course> electiveCourses,
			int electiveCreditRequirement, int totalCreditRequirement) {
	...
	public boolean containsCourse(String courseID) {
		...
		boolean inReq = requiredCourses.stream().anyMatch(c -> c.getCourseID().equalsIgnoreCase(courseID));
		if (inReq)
			return true;
		return electiveCourses.stream().anyMatch(c -> c.getCourseID().equalsIgnoreCase(courseID));
	}
	...
	public boolean isRequiredCourse(String courseID) {
		if (courseID == null || courseID.trim().isEmpty())
			return false;
		return requiredCourses.stream().anyMatch(c -> c.getCourseID().equalsIgnoreCase(courseID));
	}
	...
	public int countCompletedElectiveCredits(CreditBasedStudent student) {
		return (int) student.getCompletedEnrollments().stream()
				...
				.mapToInt(e -> e.getCourse().getCreditCount()).sum();
	}

	@Override
	public String toString() {
		return String.format("Program[%s, type=%s, required=%s, elective=%s, electiveReq=%d, totalReq=%d]", majorName,
				type, requiredCourses, electiveCourses, electiveCreditRequirement, totalCreditRequirement);
	}
}
