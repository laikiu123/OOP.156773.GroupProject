...
public class Course {
	...
	/**
	 * Mã học phần tiên quyết (prerequisite). Nếu không có thì để null hoặc chuỗi
	 * rỗng.
	 */
	...
	/**
	 * Trọng số điểm cuối kỳ, dùng để tính điểm tổng kết: ví dụ 0.6 nghĩa là 60%
	 * weight cho final, và 1 - finalWeight cho midterm. Giá trị phải nằm trong
	 * khoảng [0.0, 1.0].
	 */
	 ...
	 */
	public Course() {
		// Các trường numeric mặc định là 0, và finalWeight không hợp lệ phải được set
		// sau.
	}
	 ...
	 */
	public Course(String courseID, String courseName, int creditCount, String preCourseID, double finalWeight) {
	...
	public void setFinalWeight(double finalWeight) {
		if (finalWeight < 0.0 || finalWeight > 1.0) {
			throw new IllegalArgumentException("finalWeight must be between 0.0 and 1.0");
		}
	...
	/**
	 * Tính điểm tổng kết (final grade) theo công thức: grade = midtermScore * (1 -
	 * finalWeight) + finalScore * finalWeight
	 *
	...
	public String toString() {
		return String.format("%s - %s | Tín chỉ: %d | Tiên quyết: %s | Trọng số CK: %.2f", courseID, courseName,
				creditCount, (preCourseID == null || preCourseID.isEmpty() ? "Không" : preCourseID), finalWeight);
	}
